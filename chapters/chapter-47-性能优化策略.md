...
```typescript
// src/entrypoints/init.ts:57-160（摘要）
...
```typescript
// src/hooks/useVirtualScroll.ts:142-162（摘要）
...
```typescript
// src/ink/line-width-cache.ts:1-24
...
```typescript
// src/commands/clear/caches.ts:47-144（摘要）
...
```typescript
// src/services/api/promptCacheBreakDetection.ts:28-69
type PreviousState = {
  systemHash: number
  toolsHash: number
