...
事件类型定义位于 `src/entrypoints/sdk/coreTypes.ts`：
...
执行函数 `executeSessionStartHooks` 位于 `src/utils/hooks.ts`：
...
执行函数 `executeSessionEndHooks` 位于 `src/utils/hooks.ts`：
...
执行函数 `executePreToolHooks` 位于 `src/utils/hooks.ts`：
...
执行函数 `executePostToolHooks` 位于 `src/utils/hooks.ts`。
...
`startHookProgressInterval` 函数设置定期进度更新（`src/utils/hooks/hookEvents.ts`）：
